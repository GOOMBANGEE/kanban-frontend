...
export default function SettingTicketModal() {
  const { deleteTicket } = useDeleteTicket();
  const { ticketState, resetTicketState } = useTicketStore();
  ...
  return (
    <div
      ref={ref}
      className={
        "bg-customBlack-600 absolute top-4 right-4 z-10 mr-2 flex flex-col gap-y-1 rounded-sm px-2 py-2 text-sm"
      }
    >
      <button
